...
...first implementation is a version which does not care
about traffic lights or obstacles.

To support dbw_node, update this node to use the status of traffic lights too.

Simulator provides the exact location of traffic lights and their
current status in `/vehicle/traffic_lights` message. This message can be used to build this node
as well as to verify TL classifier.
...
class WaypointUpdater(object):
    ...
    def get_closest_waypoint_idx(self):
        ...
        # consider hyperplane defined by vector from prev to closest
        ...
        # test for accute or obtuse angle, i.e. closest wp ahead (+) or behind car (-)
        if val > 0:
    ...
    def decelerate_waypoints(self, waypoints, closest_idx):
        ...
        for i, wp in enumerate(waypoints):
            ...
            stop_idx = max(self.stopline_wp_idx - closest_idx - 4, 0)  # two waypoints back from stop line (center of car)
